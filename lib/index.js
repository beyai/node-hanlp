...
const BaseDir 		= __dirname + "/hanlp-1.3.2";
...
java.classpath.push( BaseDir + "/hanlp-1.3.2.jar" );
...
module.exports = class HANLP {
	...
	ShortSegment ( text ) {
		...
		ShortSegment.enableCustomDictionarySync( opts.CustomDict ) //使用自定义词典
	...
	NShortSegment ( text ) {
		let opts = this.options,
			NShortSegment = java.newInstanceSync('com.hankcs.hanlp.seg.NShort.NShortSegment');

		NShortSegment.enableCustomDictionarySync( opts.CustomDict ) //使用自定义词典
			.enablePlaceRecognizeSync( opts.PlaceRecognize ) //地名识别
			.enableOrganizationRecognizeSync( opts.OrgRecognize ); //机构名识别
